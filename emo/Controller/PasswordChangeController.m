...
@synthesize txtpassword, txtrepassword, txtnewpassword, btnsend, warringview2, warringview1, stype;
...
- (IBAction)btnsendPress:(id)sender {
    if (self.passwordcheck && self.passwordcheck1) {
        NSDictionary *resultDict = [ApiHandler changepassword:self.stype newpassword:self.txtnewpassword.text oldpassword:self.txtpassword.text repassword:self.txtrepassword.text];
...
- (void)textFieldDidBeginEditing:(UITextField *)textField {
    if ([textField isEqual:self.txtpassword]) {
        ...
        if ([self.txtpassword.text isEqualToString:self.txtrepassword.text]) {
            ...
            self.passwordcheck1 = false;self.
