...
- (void)returnFitRectForEveryBtn:(UIButton *)sender {
    ...
    if (whichTag == 0) {
        CGRect rect = CGRectMake(_tagsMarginLeftAndRightForSubView, _tagsMarginTopAndBottomForSubView, currentBtnSize.width+30, currentBtnSize.height);
        sender.frame = rect;
        return;
    }
    CGRect preBtnRect = [(UIButton *)[_tagsBtnArray objectAtIndex:whichTag - 1] frame]; // 上一个按钮的位置
    {
        ...
        CGRect wantSenderRect = CGRectMake(preBtnRect.origin.x + preBtnRect.size.width + _tagsMargin, preBtnRect.origin.y, currentBtnSize.width+30, currentBtnSize.height);

        if ([self isContainSubView:wantSenderRect]) {
            sender.frame = wantSenderRect;
        } else {
            sender.frame = CGRectMake(_tagsMarginLeftAndRightForSubView, preBtnRect.origin.y + preBtnRect.size.height + _tagsMargin, currentBtnSize.width+30, currentBtnSize.height);
        }
    }
}
